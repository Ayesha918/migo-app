...
function ComprehensionAssessment({ learner }) {
  ...
  const handleSelect = (letter) => setSelectedOption(letter);

  const handleNext = async () => {
    const updatedAnswers = [...answers, { question_id: currentQuestion.id, answer: selectedOption }];
  ...
  return (
    <>
      <div className={styles.header}>
        ...
        <button className={styles.speakerButtonSmall} onClick={handleSpeakPassage} type="button">📖🔊</button>
      </div>
      ...
      <div className={styles.card}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 10 }}>
          <p className={styles.questionText} style={{ marginBottom: 0 }}>{currentQuestion.question_text}</p>
          <button className={styles.speakerButtonSmall} onClick={handleSpeakQuestion} type="button">🔊</button>
        </div>
      </div>
        ...
        <div className={styles.optionsGrid} style={{ gridTemplateColumns: '1fr' }}>
          {options.map((opt) => (
            <button
              key={opt.letter}
              type="button"
              className={`${styles.optionButton} ${selectedOption === opt.letter ? styles.optionButtonSelected : ''}`}
...
export default ComprehensionAssessment;